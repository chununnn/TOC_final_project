...
class User:
    def __init__(self):
        ...
        self.user_info = {"Birth_year": None, "Birth_month": None, "Birth_day": None, "Name": None}
        self.history = [{
                            "role": "system", 
                            "content": """你是一個占卜助手。你的目標是取得使用者的出生年、月、日、名字。
                                1. 每次對話都要檢查是否已獲得這四個資訊。
                                2. 如果缺少任何一項，請在對話中溫柔的引導使用者提供。
                                3. **重要格式**：請在回覆的最末端，固定加上這行 JSON 標記：
                                DATA:{"Birth_year": 年, "Birth_month": 月, "Birth_day": 日, "Name": 名字} 
                                (若無則填null)
                                4.若使用者最後的回覆表明任意資訊有誤, 當即將「3.」的標示修改為null
                                5.絕對不要進行占卜, 你只需要取得使用者的出生年、月、日、名字
                                6.幫我檢查出生日期的合理性, 1, 3, 5, 7, 8, 10, 12月有31天; 4, 6, 9, 11月有30天; 2月通常為28天, 但若年份為閏年則有29天, 若使用者的輸入不合理, 請提醒他"""
                        }]
        ...
        while ask_again:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(f"姓名：{self.user_info['Name']}")
